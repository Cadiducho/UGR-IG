...
void _tanque::draw(_modo modo, float r1, float g1, float b1, float r2, float g2, float b2, float grosor) {
    glPushMatrix();
    if (activo[0] == 1) {
        chasis.draw(modo, r_s, g_s, b_s, r2, g2, b2, grosor);
    } else {
    ...
    if (activo[1] == 1) {
        torreta.draw(modo, r_s, g_s, b_s, r2, g2, b2, grosor);
    } else {
    ...
    if (activo[2] == 1) {
        tubo.draw(modo, r_s, g_s, b_s, r2, g2, b2, grosor);
    } else {
...
void _tanque::seleccion(float c) {
    glPushMatrix();
...
void _grua::draw(_modo modo) {
    ...
    if (activo[0] == 1) {
        drawBase(modo, r_s, g_s, b_s, 0.8, 0.7, 0, 2);
    } else {
...

    {
        ...
        if (activo[1] == 1) {
              drawBrazo(modo, r_s, g_s, b_s, 0.9, 0.65, 0, 2);
        } else {
        ...
        
        {
            ...
            if (activo[2] == 1) {
                drawAntebrazo(modo, r_s, g_s, b_s, 0.85, 0.7, 0, 2);
            } else {
            ...
            if (activo[3] == 1) {
                drawTaladro(modo, r_s, g_s, b_s,  0.5, 1, 0, 2);
            } else { 
...
void _grua::seleccion(float c) {
    glPushMatrix();
    glRotatef(this->angulo_base, 0, 1, 0);
    drawBase(SELECT, c, c, c, c, c, c, 2);
    c=c+20;
    {
        // Brazo
        glPushMatrix();
        glTranslatef(0.25, 2, 0.5);
        glRotatef(this->angulo_brazo, 0, 0, 1);
        drawBrazo(SELECT, c, c, c, c, c, c, 2);
        c=c+20;
        {
            // Antebrazo
            glPushMatrix();
            glTranslatef(2.5, 0, 0.5);
            glRotatef(this->angulo_antebrazo, 0, 0, 1);
            drawAntebrazo(SELECT, c, c, c, c, c, c, 2);
            c=c+20;
            glTranslatef(2.75, 0, -0.25);

            drawTaladro(SELECT, c, c, c, c, c, c, 2);
            glPopMatrix();
        }
...
void _grua::drawParte(_triangulos3D& parte, _modo modo, float r1, float g1, float b1, float r2, float g2, float b2, float grosor) {
    switch (modo) {
        ...
        case SELECT: parte.draw_seleccion(r1);break;
  }
}
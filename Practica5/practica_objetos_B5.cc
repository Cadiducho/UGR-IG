...
void selecionar_parte_objeto_tanque(int numeroParte) {
    //cout << "Cambiando tanque a " << numeroParte << endl;
    if (tanque.activo[numeroParte]==0) {
        tanque.activo[numeroParte] = 1;
        tanque.cambiar[numeroParte] = 1;
    } else {
        tanque.activo[numeroParte] = 0;
        tanque.cambiar[numeroParte] = 0;
    }
}

void selecionar_parte_objeto_grua(int numeroParte) {
    //cout << "Cambiando grua a " << numeroParte << endl;
    if (grua.activo[numeroParte]==0) {
        grua.activo[numeroParte] = 1;
        grua.cambiar[numeroParte] = 1;
    } else {
        grua.activo[numeroParte] = 0;
        grua.cambiar[numeroParte] = 0;
    }
}

void procesar_color(unsigned char color[3]) {
    //cout << "Procesando cambio de objeto con color " << ((int)color[0]) << " y t_objeto" << t_objeto << " "<< (t_objeto == GRUA ? "true" : "false") << endl;
    switch (color[0]) {
        case 100:
            if (t_objeto == GRUA) {
                selecionar_parte_objeto_grua(0);
            } else if (t_objeto == ARTICULADO) {
                selecionar_parte_objeto_tanque(0);
            }
            break; 
        case 120:
            if (t_objeto == GRUA) {
                selecionar_parte_objeto_grua(1);
            } else if (t_objeto == ARTICULADO) {
                selecionar_parte_objeto_tanque(1);
            }
            break; 
        case 140:
            if (t_objeto == GRUA) {
                selecionar_parte_objeto_grua(2);
            } else if (t_objeto == ARTICULADO) {
                selecionar_parte_objeto_tanque(2);
            }
            break;
        case 160:
            if (t_objeto == GRUA) {
                selecionar_parte_objeto_grua(3);
            }
            break;
    }
}
